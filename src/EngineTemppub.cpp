...
void publish_data(StatusDataWriter_var& mssgWriter, Status &mssg) {
...
int main(int argc, char *argv[]) {
    ...
    DDS::Topic_var topic;
    topic = participant->create_topic("EngineTemperature",typeName,TOPIC_QOS_DEFAULT,0,OpenDDS::DCPS::DEFAULT_STATUS_MASK);
    ...
    // File handle;
    // handle.open("./dataStore/EngineTempdata.txt",std::ios::in);
    // fstream* dataFile = handle.get();
    ifstream file;
    ...
    while (true) {
        ...
        if(matches.current_count >= 0) {
            payload.ready &= 0x00;
            setEngTemp(payload.ready);
            // srand(time(0));
            // payload.engineTemp = (rand() % 100);
            // srand(time(0));
            payload.engineTemp = float(rawData[count++]);
            //break;
        }
        ...
        if(matches.current_count >= 0) {
            publish_data(mssgWriter,payload);
        }
