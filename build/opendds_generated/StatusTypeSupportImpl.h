/* -*- C++ -*- */
/* Generated by /opt/OpenDDS/bin/opendds_idl version 3.20.0 (ACE version 6.2a_p23) running on input file /home/happikin/dev/projects/majorproject/ex2/Status.idl */
#ifndef OPENDDS_IDL_GENERATED_STATUSTYPESUPPORTIMPL_H_RB8ZQS
#define OPENDDS_IDL_GENERATED_STATUSTYPESUPPORTIMPL_H_RB8ZQS

#include <dds/Version.h>
#if !OPENDDS_VERSION_EXACTLY(3, 20, 0)
#  error "This file should be regenerated with opendds_idl"
#endif
#include <dds/DCPS/Definitions.h>

#include <dds/DdsDcpsC.h>

#include "StatusC.h"
#include "StatusTypeSupportC.h"
#include "dds/DCPS/Serializer.h"
#include "dds/DCPS/Service_Participant.h"
#include "dds/DCPS/TypeSupportImpl.h"
#include "dds/DCPS/Util.h"
#include "dds/DCPS/ValueReader.h"
#include "dds/DCPS/ValueWriter.h"
#include "dds/DCPS/XTypes/TypeObject.h"


/* Begin MODULE: CORBA */


/* End MODULE: CORBA */


/* Begin MODULE: OpenDDS */


/* End MODULE: OpenDDS */


/* Begin STRUCT: Status */

OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS { namespace DCPS {

struct Status_xtag {};
template<> const XTypes::TypeIdentifier& getMinimalTypeIdentifier<Status_xtag>();

template<> const XTypes::TypeMap& getMinimalTypeMap<Status_xtag>();

} }
OPENDDS_END_VERSIONED_NAMESPACE_DECL

OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS { namespace DCPS {

bool vread(OpenDDS::DCPS::ValueReader& value_reader,  ::Status& value);

} }
OPENDDS_END_VERSIONED_NAMESPACE_DECL

OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS { namespace DCPS {

void vwrite(OpenDDS::DCPS::ValueWriter& value_writer, const  ::Status& value);

} }
OPENDDS_END_VERSIONED_NAMESPACE_DECL

OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS { namespace DCPS {

template<> void set_default( ::Status& stru);

void serialized_size(const Encoding& encoding, size_t& size, const  ::Status& stru);

bool operator<<(Serializer& strm, const  ::Status& stru);

bool operator>>(Serializer& strm,  ::Status& stru);

void serialized_size(const Encoding& encoding, size_t& size, const NestedKeyOnly<const  ::Status>& stru);

bool operator<<(Serializer& strm, const NestedKeyOnly<const  ::Status>& stru);

bool operator>>(Serializer& strm, const NestedKeyOnly< ::Status>& stru);

void serialized_size(const Encoding& encoding, size_t& size, const KeyOnly<const  ::Status>& stru);

bool operator<<(Serializer& strm, const KeyOnly<const  ::Status>& stru);

bool operator>>(Serializer& strm, const KeyOnly< ::Status>& stru);

template <>
struct MarshalTraits< ::Status> {
  static void representations_allowed_by_type(DDS::DataRepresentationIdSeq& seq)
  {
    seq.length(4);
    seq[0] = DDS::XCDR_DATA_REPRESENTATION;
    seq[1] = DDS::XCDR2_DATA_REPRESENTATION;
    seq[2] = DDS::XML_DATA_REPRESENTATION;
    seq[3] = OpenDDS::DCPS::UNALIGNED_CDR_DATA_REPRESENTATION;
  }

  static SerializedSizeBound serialized_size_bound(const Encoding& encoding)
  {
    switch (encoding.kind()) {
    case Encoding::KIND_UNALIGNED_CDR:
      return SerializedSizeBound(25);
    case Encoding::KIND_XCDR1:
      return SerializedSizeBound();
    case Encoding::KIND_XCDR2:
      return SerializedSizeBound();
    default:
      OPENDDS_ASSERT(false);
      return SerializedSizeBound();
    }
  }

  static SerializedSizeBound key_only_serialized_size_bound(const Encoding& encoding)
  {
    switch (encoding.kind()) {
    case Encoding::KIND_UNALIGNED_CDR:
      return SerializedSizeBound(4);
    case Encoding::KIND_XCDR1:
      return SerializedSizeBound();
    case Encoding::KIND_XCDR2:
      return SerializedSizeBound();
    default:
      OPENDDS_ASSERT(false);
      return SerializedSizeBound();
    }
  }

  static bool to_message_block(ACE_Message_Block&, const  ::Status&) { return false; }
  static bool from_message_block( ::Status&, const ACE_Message_Block&) { return false; }
  static Extensibility extensibility() { return APPENDABLE; }
  static Extensibility max_extensibility_level() { return APPENDABLE; }
};
} }
OPENDDS_END_VERSIONED_NAMESPACE_DECL


/// This structure supports use of std::map with one or more keys.
struct  Status_OpenDDS_KeyLessThan {
  bool operator()(const  ::Status& v1, const  ::Status& v2) const
  {
    using ::operator<; // TAO::String_Manager's operator< is in global NS
    if (v1.ID < v2.ID) return true;
    if (v2.ID < v1.ID) return false;
    return false;
  }
};


class StatusTypeSupportImpl;

OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS {
namespace DCPS {
template <>
struct DDSTraits< ::Status> {
  typedef  ::Status MessageType;
  typedef  ::StatusSeq MessageSequenceType;
  typedef  ::StatusTypeSupport TypeSupportType;
  typedef  ::StatusTypeSupportImpl TypeSupportImplType;
  typedef  ::StatusDataWriter DataWriterType;
  typedef  ::StatusDataReader DataReaderType;
  typedef  ::Status_OpenDDS_KeyLessThan LessThanType;
  typedef OpenDDS::DCPS::KeyOnly<const  ::Status> KeyOnlyType;

  static const char* type_name() { return "Status"; }
  static bool gen_has_key() { return true; }
  static size_t key_count() { return 1; }
};
} // namespace DCPS
} // namespace OpenDDS
OPENDDS_END_VERSIONED_NAMESPACE_DECL


class  StatusTypeSupportImpl
  : public virtual OpenDDS::DCPS::LocalObject<StatusTypeSupport>
  , public virtual OpenDDS::DCPS::TypeSupportImpl
{
public:
  typedef OpenDDS::DCPS::DDSTraits<Status> TraitsType;
  typedef OpenDDS::DCPS::MarshalTraits<Status> MarshalTraitsType;
  typedef StatusTypeSupport TypeSupportType;
  typedef StatusTypeSupport::_var_type _var_type;
  typedef StatusTypeSupport::_ptr_type _ptr_type;

  StatusTypeSupportImpl() {}
  virtual ~StatusTypeSupportImpl() {}

  virtual ::DDS::DataWriter_ptr create_datawriter();
  virtual ::DDS::DataReader_ptr create_datareader();
#ifndef OPENDDS_NO_MULTI_TOPIC
  virtual ::DDS::DataReader_ptr create_multitopic_datareader();
#endif /* !OPENDDS_NO_MULTI_TOPIC */
#ifndef OPENDDS_NO_CONTENT_SUBSCRIPTION_PROFILE
  virtual const OpenDDS::DCPS::MetaStruct& getMetaStructForType();
#endif /* !OPENDDS_NO_CONTENT_SUBSCRIPTION_PROFILE */
  virtual bool has_dcps_key();
  const char* default_type_name() const;

  void representations_allowed_by_type(::DDS::DataRepresentationIdSeq& seq);

  virtual const OpenDDS::XTypes::TypeIdentifier& getMinimalTypeIdentifier() const;
  virtual const OpenDDS::XTypes::TypeMap& getMinimalTypeMap() const;

  virtual const OpenDDS::XTypes::TypeIdentifier& getCompleteTypeIdentifier() const;
  virtual const OpenDDS::XTypes::TypeMap& getCompleteTypeMap() const;

  virtual OpenDDS::DCPS::Extensibility getExtensibility() const;

  static StatusTypeSupport::_ptr_type _narrow(CORBA::Object_ptr obj);
};

#ifndef OPENDDS_NO_CONTENT_SUBSCRIPTION_PROFILE
OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS { namespace DCPS {

class MetaStruct;

template<typename T>
const MetaStruct& getMetaStruct();

template<>
const MetaStruct& getMetaStruct< ::Status>();
bool gen_skip_over(Serializer& ser,  ::Status*);

} }
OPENDDS_END_VERSIONED_NAMESPACE_DECL

#endif

/* End STRUCT: Status */
OPENDDS_BEGIN_VERSIONED_NAMESPACE_DECL
namespace OpenDDS { namespace DCPS {

} }
OPENDDS_END_VERSIONED_NAMESPACE_DECL

#endif /* OPENDDS_IDL_GENERATED_STATUSTYPESUPPORTIMPL_H_RB8ZQS */
